...
- Ran `launchctl setenv OLLAMA_HOST "0.0.0.0"` + `brew services restart ollama` directly in Terminal. Confirmed via live curl: `http://10.10.1.201:11434/api/tags` now returns HTTP 200 — reachable from off-host.
...
- Confirmed visible over the LAN via live curl to `http://10.10.1.201:11434/api/tags`, not just locally.
